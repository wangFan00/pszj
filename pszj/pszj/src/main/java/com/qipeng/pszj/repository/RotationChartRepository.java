...
@Repository
public interface RotationChartRepository extends JpaRepository<RotationChart,String>{
	/*
    * 查询所有要展示的轮播图
    * */
    @Query(value ="select r from RotationChart r where r.state=:state")
    List<RotationChart> find(@Param("state")int state);
}
